...
const SignIn: React.FC<SignInProps> = ({ onSwitchToRegister, onLogin }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      <h2 className="text-3xl font-bold text-teal-600 mb-8">Sign in to Attendance</h2>
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-5">
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg bg-white focus:outline-none focus:border-teal-500 transition-all"
          >
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  maxLength={10}
                  className="flex-1 px-4 py-3 border-2 border-gray-200 rounded-lg bg-white focus:outline-none focus:border-teal-500 transition-all"
                />
                <button
                  type="button"
                  onClick={handleSendOtp}
                  disabled={sendingOtp || !loginData.mobileNumber || !/^[0-9]{10}$/.test(loginData.mobileNumber)}
                  className="px-4 py-3 bg-gray-600 text-white rounded-lg text-sm font-medium whitespace-nowrap hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
        ...
        <button
          type="submit"
          className="w-full py-3 px-4 bg-teal-600 text-white font-semibold rounded-lg tracking-wide hover:bg-teal-700 transition-all duration-300"
        >
          SIGN IN
        </button>
      </form>
